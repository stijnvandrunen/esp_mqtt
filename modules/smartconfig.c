/******************************************************************************
 * Copyright 2014-2015  (By younger, Stijn van Drunen)
 *
...
{  
  ...
  ets_uart_printf("[smart] calcrc_bytes: %02x \n",crc);
  return crc;
}
...
smart_analyze(uint8* buf,int len,int channel)
{
      ...
      if( os_memcmp(smart_mac,buf,6) == 0 ) {
        ...
        if( os_memcmp(smart_mac,buf,6) == 0 ){
            if( len == 0 ){
              ...
              smart_onbyte_Idx = 0;
              smart_onbyte[smart_onbyte_Idx++]=len;
              ets_uart_printf("[smart] resync  into SMART_CH_LOCKING\n");
            }else{
              smart_onbyte[smart_onbyte_Idx++]=len; 
            }
            smart_lock_time=system_get_time();
  ...
  if(smart_onbyte_Idx>=10)
  {
    ...
    ets_uart_printf("[smart] Decode ret=%d\n",funret);
...
*******************************************************************************/
void ICACHE_FLASH_ATTR
wifi_promiscuous_rx(uint8 *buf, uint16 nolen)
{
 // ets_uart_printf("something receive...\n");
  uint16 i;
  uint16 len;
  ...
  //ets_uart_printf("[smart] len = %d,buf = %02x %02x\n",len, buf[0],buf[1]);}
  ...
  if ((buf[0] == 0x08 || buf[0] == 0x88) && ((buf[1] & 0x02) == 0x02) /*&& (len!=116) && (len!=98)*/) {
	 // ets_uart_printf("[smart] len = %d,buf = %02x %02x\n",len, buf[0],buf[1]);
...
void ICACHE_FLASH_ATTR channelScan(void *arg) {
  uint8_t i;

  if(smart_channel > 0 && smart_lock_flag == 1) {
    wifi_set_channel(smart_channel);
    ets_uart_printf("[smart] Locked to Smartlink channel %d\n", smart_channel);
    return;
  }
  
  for (i = current_channel; i < 15; i++) {
    if ((channel_bits & (1 << i)) != 0) {
      current_channel = i + 1;
      wifi_set_channel(i);
      ets_uart_printf("[smart] Current channel %d\n", i);
      os_timer_arm(&channel_timer, SCAN_TIME, 0);
      break;
    }

    // Couldn't find anything, start over.
    if (i == 14) {
      current_channel = 1;
      os_timer_arm(&channel_timer, SCAN_TIME, 0);
      break;
  ...
  if (status == OK) {
    ...
    while (bss != NULL) {
      ...
      if (bss->channel != 0) {
        ...
        ets_uart_printf("[smart] ssid %s, channel %d, authmode %d, rssi %d\n",
    ...
    os_timer_setfn(&channel_timer, channelScan, NULL);
    os_timer_arm(&channel_timer, SCAN_TIME, 0);
  } else {
	  ets_uart_printf("[smart] err, scan status %d\n", status);
  }
}
...
{
  char *ssid, *password, *mqtthost;
  
  wifi_promiscuous_enable(0);

  ssid = strtok(buf, "\n");
  password = strtok(NULL, "\n");
  mqtthost = strtok(NULL, "\n");

  ets_uart_printf("*************************************\n");
  ets_uart_printf("Get the smartconfig result:\n");
  ets_uart_printf("%s\n",ssid);
  ets_uart_printf("%s\n",password);
  ets_uart_printf("%s\n",mqtthost);
  ets_uart_printf("*************************************\n");
  ...
  os_sprintf(sysCfg.sta_pwd, "%s", password);
  os_sprintf(sysCfg.mqtt_host, "%s", mqtthost);
  CFG_Save();
